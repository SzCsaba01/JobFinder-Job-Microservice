...
public class ExternalSourceVisitClickRepository : IExternalSourceVisitClickRepository
{
    private readonly DataContext _dataContext;

    public ExternalSourceVisitClickRepository(DataContext dataContext)
    {
        _dataContext = dataContext;
    }

    public async Task<List<ExternalSourceVisitClickEntity>> GetExternalSourceVisitClicksWhereEmailNotSentAsync()
    {
        return await _dataContext.ExternalSourceVisitClicks
            .Where(x => !x.isFeedbackMailSent)
            .Include(x => x.Job)
            .AsNoTracking()
            .ToListAsync();
    }
    ...
    {
        var query = _dataContext.ExternalSourceVisitClicks
    ...
    public async Task<ExternalSourceVisitClickEntity?> GetExternalSourceVisitClickByUserProfileIdAndJobIdAsync(Guid userProfileId, Guid jobId)
    {
        return await _dataContext.ExternalSourceVisitClicks
    ...
    public async Task<List<ExternalSourceVisitClickEntity>?> GetExternalSourceVisitClicksByUserProfileIdAsync(Guid userProfileId)
    {
        return await _dataContext.ExternalSourceVisitClicks
            .Where(x => x.UserProfileId == userProfileId)
            .AsNoTracking()
    ...
    public async Task<List<ExternalSourceVisitClickEntity>?> GetExternalSourceVisitClicksForRecommendationsByUserProfileIdAsync(Guid userProfileId)
    {
        return await _dataContext.ExternalSourceVisitClicks
            .Where(x => x.UserProfileId == userProfileId)
            .OrderByDescending(x => x.ClickDate)
            .Take(5)
            .AsNoTracking()
            .ToListAsync();
    }

    public async Task AddExternalSourceVisitClickAsync(ExternalSourceVisitClickEntity externalSourceVisitClick)
    {
        await _dataContext.ExternalSourceVisitClicks.AddAsync(externalSourceVisitClick);
        await _dataContext.SaveChangesAsync();
    }

    public async Task UpdateExternalSourceVisitClicksAsync(List<ExternalSourceVisitClickEntity> externalSourceVisitClicks)
    {
        _dataContext.UpdateRange(externalSourceVisitClicks);
        await _dataContext.SaveChangesAsync();
    }

    public async Task DeleteExternalSourceVisitClicksAsync(List<ExternalSourceVisitClickEntity> externalSourceVisitClicks)
    {
        _dataContext.RemoveRange(externalSourceVisitClicks);
        await _dataContext.SaveChangesAsync();
    }
}
